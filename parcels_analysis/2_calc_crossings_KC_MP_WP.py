...
Names:
- NS: North Section (another script!) (in final manuscript: North-of-Curacao NC)
- WP: West Point
- MP: Mid Point
- KC: Klein Curacao
- SS: South Section (another script!) (in final manuscript: South-of-Curacao SC)
...
if __name__ == "__main__":
    ...
    crossings_KC_ds = xr.Dataset()
    crossings_WP_ds = xr.Dataset()
    crossings_MP_ds = xr.Dataset()

    # Process in batches to manage memory
    batch_size = 19293 # number of particles released at each seeding time
    ...
    for batch_num in range(num_batches):
        ...
        for section_name, crossings in batch_crossings.items():
            if crossings:  # If we have crossings for this section
                ...
                if section_name == "KC":
                    crossings_KC_ds = xr.concat([crossings_KC_ds, temp_ds], dim='crossing') if 'crossing' in crossings_KC_ds.dims else temp_ds
                elif section_name == "WP":
                    crossings_WP_ds = xr.concat([crossings_WP_ds, temp_ds], dim='crossing') if 'crossing' in crossings_WP_ds.dims else temp_ds
                elif section_name == "MP":
                    crossings_MP_ds = xr.concat([crossings_MP_ds, temp_ds], dim='crossing') if 'crossing' in crossings_MP_ds.dims else temp_ds
        
        # Save results only for the last batch (because the data is accumulating)
        if batch_num == num_batches - 1:
            if 'crossing' in crossings_KC_ds.dims:
                crossings_KC_ds.to_netcdf(f'../parcels_analysis/crossings_calculations/FINAL/xrcrossings_KC_{part_month}_ALL.nc')
            if 'crossing' in crossings_WP_ds.dims:
                crossings_WP_ds.to_netcdf(f'../parcels_analysis/crossings_calculations/FINAL/xrcrossings_WP_{part_month}_ALL.nc')
            if 'crossing' in crossings_MP_ds.dims:
                crossings_MP_ds.to_netcdf(f'../parcels_analysis/crossings_calculations/FINAL/xrcrossings_MP_{part_month}_ALL.nc')
